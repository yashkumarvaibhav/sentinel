  ...
  return (
    <section className="bg-surface-raised flex flex-col gap-4 rounded-xl p-5">
      <div className="flex items-baseline justify-between gap-4">
        ...
        <span className="text-ink-muted text-[11px] tracking-wider uppercase">Real</span>
      </div>

      {health.state === 'loading' && <p className="text-ink-muted text-sm">Checking…</p>}

      {health.state === 'error' && (
        <p className="text-sm">
          <span className="text-residual">Gateway unreachable</span>
          <span className="text-ink-muted"> — {health.error}</span>
        </p>
      )}
      ...
      <p className="text-ink-muted border-ink-muted/15 border-t pt-3 text-xs">
        {version.state === 'ok'
